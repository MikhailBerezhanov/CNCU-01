...
static uint16_t CalculatePrescaler (void)
{
	...
	if (APBPrescTable[(RCC->CFGR & RCC_CFGR_PPRE2)>>RCC_CFGR_PPRE2_Pos])
		return ((CNCU.Config.PCLK2 * 2) / tick_freq - 1);
	else
		return (CNCU.Config.PCLK2 / tick_freq - 1);
}
...
void Leds_ON (ELEDS ltype)
{
	switch (ltype)
	{
		case ERR_LED: HAL_GPIO_WritePin(GPIOE, GPIO_PIN_3, GPIO_PIN_RESET);
			break;
		
		case CAN_LED: HAL_GPIO_WritePin(GPIOE, GPIO_PIN_4, GPIO_PIN_RESET);
			break;
		
		case GEN_LED: HAL_GPIO_WritePin(GPIOE, GPIO_PIN_5, GPIO_PIN_RESET);
			break;
...
void Leds_OFF (ELEDS ltype)
{
	switch (ltype)
	{
		case ERR_LED: HAL_GPIO_WritePin(GPIOE, GPIO_PIN_3, GPIO_PIN_SET);
			break;
		
		case CAN_LED: HAL_GPIO_WritePin(GPIOE, GPIO_PIN_4, GPIO_PIN_SET);
			break;
		
		case GEN_LED: HAL_GPIO_WritePin(GPIOE, GPIO_PIN_5, GPIO_PIN_SET);
			break;
...
void Leds_Toggle (ELEDS ltype)
{
	switch (ltype)
	{
		case ERR_LED: HAL_GPIO_TogglePin(GPIOE, GPIO_PIN_3);
			break;
		
		case CAN_LED: HAL_GPIO_TogglePin(GPIOE, GPIO_PIN_4);
			break;
		
		case GEN_LED: HAL_GPIO_TogglePin(GPIOE, GPIO_PIN_5);
			break;
