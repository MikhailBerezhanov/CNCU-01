...
extern volatile uint32_t fl_timer;

/** TODO: Make timer-creating interface instead of hardcode
...
static uint16_t CalculatePrescaler (ERESOL resolution)
{
	...
	if (APBPrescTable[(RCC->CFGR & RCC_CFGR_PPRE2)>>RCC_CFGR_PPRE2_Pos])
		presc = (HAL_RCC_GetPCLK2Freq() * 2) / tick_freq - 1;
	else
		presc = HAL_RCC_GetPCLK2Freq() / tick_freq - 1;
